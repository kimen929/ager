...
* `auto_get_exchange_rate.php`. This file is main wordpress plugin file that extract the exchange rate from the website of [Back of China](http://www.boc.cn/sourcedb/whpj/enindex.htm). 
* `simple_html_dom.php`. This file is an open source library that can extract contents from HTML in a single line. See details by click [this](http://simplehtmldom.sourceforge.net/). 
...
* After this plugin installed in wordpress and activated, the program will automatically check the newest foreign exchange rate *hourly* from [Back of China](http://www.boc.cn/sourcedb/whpj/enindex.htm), and add the result of `CNY/JPY` to the wordpress database table named `$wpdb->prefix.ager_exchange_rate`.
...
Because the exchange rate data is from Back of China, it is strictly prohibited to use the data in *Commercial Purpose*.
